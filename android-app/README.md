...
- 使用“角色 / 因子 / 已选 / 结果”四页式手机界面；未登录时显示官方登录页，登录有效时先显示 App 加载页，再直接进入搜索器。
...
脚本输出为`app/build/outputs/apk/debug/uma-seed-searcher-android-v0.1.4-debug.apk`。调试包只用于本机安装测试，正式分发前需要改用单独保管的发布签名。
