(function seedOptimizerContentScript() {
  ...
  shadow.innerHTML = `
    <style>
      ...
      * { box-sizing:border-box; }
      ...
      .factor-option { min-height:52px; display:flex; align-items:flex-start; justify-content:space-between; gap:7px; border:1px solid transparent; border-radius:10px; padding:8px 10px; color:var(--ink); background:#fff; text-align:left; }
      ...
      h1 { font-size:var(--fs-title); } h2 { font-size:var(--fs-lg); }
      .score-value { font-size:var(--fs-score); }
  ...
  function renderRecognitionAutoSummary(warnings) {
    ...
    const parts = [];
    if (ignored.length) {
      const ignoredText = [...new Set(ignored
        .map((item) => String(item?.text || "").replace(/\s+/g, " ").trim())
        .filter(Boolean))];
      parts.push(`<p>忽略 ${ignored.length} 处无效文字</p><p>${escapeHtml(ignoredText.join("、") || "未提供原文")}</p>`);
    }
    if (duplicates.length) parts.push(`<p>合并 ${duplicates.length} 个重复因子。</p>`);
    if (other.length) parts.push(`<p>${other.length} 条其他提示：${escapeHtml(recognitionExamples(other, recognitionMessage))}。</p>`);
    return `<div class="recognition-issue-summary">${parts.join("")}</div>`;
  }
